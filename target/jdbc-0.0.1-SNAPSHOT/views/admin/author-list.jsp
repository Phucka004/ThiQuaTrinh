...
<script>
    ...
    $(document).ready(function () {
        $('#bookTable').DataTable({
            ...
            "columns": [
                {"data": "author_id"},        // ID sách
                {"data": "author_name"},         // Tên sách
                {"data": "date_of_birth"},   // Mô tả sách
                {
                    "data": null,
                    "orderable": false,
                    "render": function (data, type, row) {
                        console.log("Row data:", row.bookid); // Kiểm tra dữ liệu của từng hàng
                        return '<button class="detail-btn" data-id=' + row.author_id + ' > Chi tiết </button>';
                    }
     ...
     // Xử lý sự kiện khi nhấn nút Chi tiết
        $('#bookTable').on('click', '.detail-btn', function () {
            ...
            window.location.href = "/JPA-KT/book-details?author_id=" + bookId;
        });
